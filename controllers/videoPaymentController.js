...
exports.processVideoPayment = async (req, res, next) => {
    ...
    params["CALLBACK_URL"] = `http://${req.get("host")}/api/v1/e-content/video/callback/user/${uid}/playlist/${pid}/video/${vid}/ins/${fid}`;
            ...
            var post_req = https.request(options, function (post_res) {
                ...
                post_res.on('end', function () {
                    ...
                    if(status === 'TXN_SUCCESS'){
                        ...
                        res.redirect(`http://localhost:3000/user/${uid}/e-content`)
                    }
                    else{
...
const userVideoUpdated = async (userId, playlistId, financeId, videoId, statusType, tx_amounts) => {
    try{
        const user = await User.findById({_id: userId})
