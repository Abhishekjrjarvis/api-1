...
exports.processApplicationPayment = async (req, res, next) => {
    ...
    params["CALLBACK_URL"] = `http://${req.get("host")}/api/v1/application/callback/${uid}/apply/${aid}/ins/${iid}/finance/${fid}`;
            ...
            var post_req = https.request(options, function (post_res) {
                ...
                post_res.on('end', function () {
                    ...
                    if(status === 'TXN_SUCCESS'){
                        ...
                        res.redirect(`http://107.20.124.171:3000/user/${uid}/insjoinandapply/${iid}/application-apply/${aid}`)
                    }
                    else{
...
const userApplicationUpdated = async (userId, applyId, insId, financeId, statusType, tx_apAmount) => {
    try{
        const user = await User.findById({_id: userId})
        ...
        institute.insApplicationBalance = institute.insApplicationBalance + parseInt(tx_apAmount)
        finance.financeApplicationBalance = finance.financeApplicationBalance + parseInt(tx_apAmount)
