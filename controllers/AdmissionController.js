...
exports.processAdmissionPayment = async (req, res, next) => {
    ...
    params["CALLBACK_URL"] = `http://${req.get("host")}/api/v1/admission/callback/${uid}/apply/${aid}/ins/${iid}/finance/${fid}`;
...
const userAdmissionUpdated = async (userId, applyId, insId, financeId, statusType, tx_adAmount) => {
    try{
        const user = await User.findById({_id: userId})
        ...
        institute.insAdmissionBalance = institute.insAdmissionBalance + parseInt(tx_adAmount)
        finance.financeAdmissionBalance = finance.financeAdmissionBalance + parseInt(tx_adAmount)
