...
exports.processPayment = async (req, res, next) => {
    ...
    params["CALLBACK_URL"] = `http://${req.get("host")}/api/v1/callback/pay/${fiid}/${uid}/student/${sid}/fee/${fid}`;
            ...
            var post_req = https.request(options, function (post_res) {
                ...
                post_res.on('end', function () {
                    ...
                    if(status === 'TXN_SUCCESS'){
                        ...
                        res.redirect(`http://localhost:3000/user/${uid}/student/fee/${sid}`)
                    }
                    else{
...
const studentPaymentUpdated = async (financeId, studentId, feeId, statusType, tx_amount) => {
    try{
        const student = await Student.findById({ _id: studentId });
        const finance = await Finance.findById({ _id: financeId }).populate({ path: 'institute'})
...
exports.processEContentPayment = async (req, res, next) => {
    ...
    params["CUST_ID"] = process.env.PAYTM_CUST_ID;
    params["TXN_AMOUNT"] = amount;
    params["CALLBACK_URL"] = `http://${req.get("host")}/api/v1/e-content/callback/user/${uid}/playlist/${pid}/ins/${fid}`;
    let paytmChecksum = paytm.generateSignature(params, process.env.PAYTM_MERCHANT_KEY);
    paytmChecksum.then(function (checksum) {
        let paytmParams = {
            ...params,
            ...
            var post_req = https.request(options, function (post_res) {
                ...
                post_res.on('end', function () {
                    ...
                    if(status === 'TXN_SUCCESS'){
                        ...
                        res.redirect(`http://localhost:3000/user/${uid}/e-content/playlist/${pid}`)
                    }
                    else{
                        res.redirect(`http://localhost:3000/`)
                    }
...
const userEContentUpdated = async (userId, playlistId, financeId, statusType, tx_amounts) => {
    try{
        const user = await User.findById({_id: userId})
        const playlist = await Playlist.findById({_id: playlistId}).populate({path: 'elearning'})
        const finance = await Finance.findById({_id: financeId}).populate({path: 'institute'})
        ...
        playlist.elearning.bankBalance = playlist.elearning.bankBalance + parseInt(tx_amounts)
        finance.financeEContentBalance = finance.financeEContentBalance + parseInt(tx_amounts)
        finance.institute.insEContentBalance = finance.institute.insEContentBalance + parseInt(tx_amounts)
        await user.save();
        await playlist.save();
...
exports.processIdCardPayment = async (req, res, next) => {
    ...
    params["CUST_ID"] = process.env.PAYTM_CUST_ID;
    params["TXN_AMOUNT"] = amount;
    params["CALLBACK_URL"] = `http://${req.get("host")}/api/v1/callback/ins/${id}/batch/${batchId}`;
    let paytmChecksum = paytm.generateSignature(params, process.env.PAYTM_MERCHANT_KEY);
    paytmChecksum.then(function (checksum) {
        let paytmParams = {
            ...params,
            ...
            var post_req = https.request(options, function (post_res) {
                ...
                post_res.on('end', function () {
                    ...
                    if(status === 'TXN_SUCCESS'){
                        ...
                        res.redirect(`http://localhost:3000/ins/${id}/student/card`)
                    }
                    else{
                        res.redirect(`http://localhost:3000/`)
                    }
...
const userIdCardUpdated = async (insId, bid, statusType, tx_iAmounts) => {
    try {
        ...
        // if(admin.instituteIdCardBatch.length >=1 && admin.instituteIdCardBatch.includes(String(bid))){
        //   console.log('yes')
        // }
        // else{
        // }
...
//     const { amount, aid, sid, fid, uid } = req.body;
//     var params = {};
//     params["MID"] = process.env.PAYTM_MID;
//     params["WEBSITE"] = process.env.PAYTM_WEBSITE;
//     params["CHANNEL_ID"] = process.env.PAYTM_CHANNEL_ID;
...
//     params["CUST_ID"] = process.env.PAYTM_CUST_ID;
//     params["TXN_AMOUNT"] = amount;
//     params["CALLBACK_URL"] = `http://${req.get("host")}/api/v1/application/callback/${aid}/student/${sid}/pay/${fid}/user/${uid}`;
//     let paytmChecksum = paytm.generateSignature(params, process.env.PAYTM_MERCHANT_KEY);
//     paytmChecksum.then(function (checksum) {
//         let paytmParams = {
//             ...params,
...
//     const { aid, sid, fid, uid } = req.params;
//     let paytmChecksum = req.body.CHECKSUMHASH;
//     delete req.body.CHECKSUMHASH;
//     let isVerifySignature = paytm.verifySignature(req.body, process.env.PAYTM_MERCHANT_KEY, paytmChecksum);
//     if (isVerifySignature) {
...
//                     if(status === 'TXN_SUCCESS'){
//                         addApplicationPayment(body, aid, sid, uid);
//                         userApplicationUpdated(aid, sid, fid, status, price)
//                         res.redirect(`http://localhost:3000/`)
//                     }
//                     else{
//                         res.redirect(`http://localhost:3000/`)
...
// const addApplicationPayment = async (data, applyId, studentId, userId) => {
//     try {
//         const student = await Student.findById({_id: studentId })
//         const apply = await new ApplyPayment(data);
//         apply.studentId = studentId
//         apply.applyId = applyId
//         apply.userId = userId
//         student.applyList.push(apply)
//         await apply.save()
//         await student.save()
//     } catch (error) {
...
// const userApplicationUpdated = async (applyId, studentId, financeId, statusType, price) => {
//     try{
        // const user = await User.findById({_id: userId})
